...
void GameChoice::Init() 
{
    ...
    // Game 1 button
    InitText(m_butinButton, "Butin", sf::Vector2f(width / 2, height / 2 - 25.f), 20);

    // Checkers game button
    InitText(m_checkersButton, "Les Dames", sf::Vector2f(width / 2, height / 2), 20);

    // Game 3 button
    InitText(m_bulltrickerButton, "Bulltricker", sf::Vector2f(width / 2, height / 2 + 25.f), 20);
...
void GameChoice::Update()
{
    ...
    if(m_isButinButtonPressed)
    {
        ...
        m_context->m_states->Add(std::make_unique<Butin>(m_context), true);
